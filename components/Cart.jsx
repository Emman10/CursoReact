...
const Cart = () => {
  ...
  const removeItem = (id) => setCart(cart.filter(alfajor => alfajor.id !== id))
  ...
  if (cart.length === 0) {
    return (
      <div className='messageCartEmpty'>
        <Heading textAlign="center" mb={4} fontFamily={"Maven Pro, sans-serif"}>No ha seleccionado ningún alfajor para comprar</Heading>
        <Link to={"/catalogue"}>
            ...
            >
              ...
              <Stack>
                <CardBody>
                  ...
                  <br />
                  <Text as='b'>Cantidad: </Text>{alfajor.quantity}
                  <br />
                  <Text as='b'>Precio por unidad: </Text>${alfajor.price}
                  <br />
                  <Text as='b'>Precio subtotal: </Text>${alfajor.price * alfajor.quantity}
                </CardBody>
                <CardFooter>
                  <Button colorScheme='#f9e79f'>
                  <DeleteIcon w={6} h={6} color='red.500' onClick={() => removeItem(alfajor.id)}/>
                  </Button>
                </CardFooter>
              </Stack>
            </Card>
